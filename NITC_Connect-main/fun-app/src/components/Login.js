...
export default function Login()
  ...
  {
    const [checkstate,setCheckState]=useState({checkBit:0,
    message:"Use Your Nitc Email ID"
    });


    const [values,setValues]=useState({email:null,password:null})
    
    let inputUpdate =(event)=>{

      setValues({email:event.target.value
                
      });
    }
    let inputUpdate2 =(event)=>{
    ...
    {
      // alert("fill Details");
      setCheckState({checkBit:1,
        message:"Fill up the Email!"
      });
    ...
    else if(password==="")
      {
        // alert("fill Details");
        setCheckState({checkBit:1,
          message:"Fill up the Password!"
        });
      
      }

    else if(email.search("@nitc.ac.in")===-1){
